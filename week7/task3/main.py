from fastapi import FastAPI, Request, Body, Form, Query, HTTPException
...
# Name Update API (PATCH)
@app.patch("/api/member")
async def update_member_name(request: Request, new_name: dict = Body(...)):
    if not request.session.get("SIGNED_IN", False):
    ...
    try:
        ...
        conn.commit()
        request.session["username"] = new_name["name"]
        return JSONResponse(content={"ok": True})
    except mysql.connector.Error as err:
        print("update_member_name 中的資料庫錯誤:", err)
        conn.rollback()
